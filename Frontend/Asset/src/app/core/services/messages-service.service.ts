...
export class MessagesServiceService {
    ...
    confirmDeActivate = () => {
        let confirmBox = new ConfirmBoxInitializer();
        confirmBox.setTitle('Are you sure?');
        confirmBox.setMessage('Confirm to DeActive this element !');
        confirmBox.setButtonLabels('Yes', 'No');
    ...
    toastNotification = (title: string, message: string, isError: boolean) => {
        ...
        newToastNotification.setConfig({
            layoutType: isError ? DialogLayoutDisplay.SUCCESS : DialogLayoutDisplay.DANGER, // SUCCESS | INFO | NONE | DANGER | WARNING
    ...
    alertBox(message: string, isError: boolean) {
        let alertBox = new ConfirmBoxInitializer();
